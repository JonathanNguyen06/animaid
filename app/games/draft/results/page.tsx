...
export default function DraftResultsPage() {
    ...
    useEffect(() => {
        ...
        try {
            const parsedResult =
                JSON.parse(savedResult) as DraftResult;
     ...
     *
     * S / S+ characters receive a longer
     * reveal window before moving on.
     */
    useEffect(() => {
        if (!result) return;
        if (revealPhase !== "lineup") return;

        if (revealedCount >= gridPicks.length) {
    ...
    /*
     * Team Power increases as each
     * lineup card is revealed.
     */
                                    ...
                                    >
                                        {isRevealed ? (
                                            ...
                                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-black via-purple-950/40 to-black">
                                                ...
                                                <p className="relative text-4xl text-pink-300/25">
                                                    {getPositionIcon(
                                                        pick.position
                                                    )}
                                                </p>

                                                <p className="relative mt-4 text-xs font-black uppercase tracking-[0.25em] text-white/25">
                                                    {
                                                        pick.position
                                                    }
                                                </p>
